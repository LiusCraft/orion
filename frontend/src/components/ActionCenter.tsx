...
import { Card, List, Button, Modal, message, Space, Tag, Progress, Row, Col } from 'antd'
import { 
  RollbackOutlined, 
  PauseOutlined, 
  PlayCircleOutlined,
...
const ActionCenter: React.FC = () => {
  ...
  const handleAction = (action: Action, approve: boolean) => {
  ...
  const confirmAction = () => {
    if (currentAction) {
      message.success(`已${currentAction.type === 'rollback' ? '回滚' : '暂停'}操作`)
    ...
    <div style={{ padding: '24px' }}>
                  ...
                  <List.Item.Meta
                    ...
                    description={
                      <div>
                        ...
                        <div>时间: {new Date(action.requestedAt).toLocaleString()}</div>
                      </div>
                    }
                  />
                </List.Item>
      ...
      <Card title="操作统计" style={{ marginTop: '16px' }}>
        <Row gutter={[16, 16]}>
          ...
          <Col xs={12} sm={6}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#52c41a' }}>98%</div>
              <div>成功率</div>
            </div>
          </Col>
          <Col xs={12} sm={6}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#fa8c16' }}>45s</div>
              <div>平均耗时</div>
            </div>
          </Col>
          <Col xs={12} sm={6}>
      ...
      >
        {currentAction && (
          <div>
            <p>是否确认执行以下操作？</p>
            <p><strong>操作类型:</strong> {getActionText(currentAction.type)}</p>
            <p><strong>任务ID:</strong> {currentAction.taskId}</p>
            <p><strong>请求人:</strong> {currentAction.requestedBy}</p>
