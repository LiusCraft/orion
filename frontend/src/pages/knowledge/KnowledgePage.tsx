...
const KnowledgePage: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<KnowledgeDocument | null>(
    null,
  );
  ...
  const toggleLikeMutation = useMutation({
    mutationFn: ({ id, isLiked }: { id: string; isLiked: boolean }) =>
      isLiked
        ? knowledgeService.unlikeDocument(id)
        : knowledgeService.likeDocument(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["knowledge-documents"] });
    },
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "author",
      key: "author",
      render: (author: UserInfo | undefined) => author?.displayName || "未知",
    },
    {
      title: "更新时间",
      dataIndex: "updatedAt",
    ...
    <div>
      ...
      <Tabs defaultActiveKey="manage">
        ...
        <TabPane tab="标签管理" key="tags">
          <Card title="标签统计">
            <Space wrap>
              {Object.entries(statistics?.tagCounts || {}).map(
                ([tag, count]) => (
      ...
      >
        {selectedItem && (
          <div>
            ...
            <Divider />
            <Text type="secondary">
              作者: {selectedItem.author?.displayName || "未知"} | 创建:{" "}
              {new Date(selectedItem.createdAt).toLocaleDateString("zh-CN")} |
              更新:{" "}
              {new Date(selectedItem.updatedAt).toLocaleDateString("zh-CN")} |
